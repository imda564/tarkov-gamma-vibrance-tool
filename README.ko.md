...
- `tarkov-settings.config.json`이 손상되거나 잘못된 경우 앱이 크래시하던 문제 (이제는 경고 후 기본값으로 리셋)
...
**창을 닫거나 트레이 아이콘에서 종료할 때 `tarkov-settings.config.json`에 설정이 저장됩니다.**

## 프로필
좌측 하단 입력창에 이름을 입력하고 **Save** / **Load** / **Del** 버튼으로 밝기/대비/감마/채도 값을 여러 세트로 저장하고 불러올 수 있습니다 (예: 게임별, 시간대별 프로필). 프로필은 다른 설정과 함께 `tarkov-settings.config.json`에 저장됩니다.
